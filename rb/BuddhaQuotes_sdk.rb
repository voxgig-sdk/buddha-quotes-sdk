...
require_relative 'BuddhaQuotes_types'


class BuddhaQuotesSDK
  ...
  def initialize(options = {})
    ...
    utility = BuddhaQuotesUtility.new
    @_utility = utility
    ...
    feature_opts = BuddhaQuotesHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BuddhaQuotesHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BuddhaQuotesFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BuddhaQuotesUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BuddhaQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BuddhaQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BuddhaQuotesHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BuddhaQuotesError.new(
        "#{op}_allow",
        "BuddhaQuotesSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BuddhaQuotesHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BuddhaQuotesError.new(
        "graphql_error", "BuddhaQuotesSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BuddhaQuotesSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
