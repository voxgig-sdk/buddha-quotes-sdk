...
module BuddhaQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BuddhaQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = BuddhaQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BuddhaQuotesResponse.new(fetched)
    else
