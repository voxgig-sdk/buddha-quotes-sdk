# BuddhaQuotes Ruby SDK



The Ruby SDK for the BuddhaQuotes API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Random` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = BuddhaQuotesSDK.test(testopts, sdkopts)
```
...
raise a `BuddhaQuotesError` (a `StandardError` subclass), so wrap
...
```
rb/
├── BuddhaQuotes_sdk.rb       -- Main SDK module
...
The main module (`BuddhaQuotes_sdk`) exports the SDK class
