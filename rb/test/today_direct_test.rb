...
class TodayDirectTest < Minitest::Test
  def test_direct_load_today
    setup = today_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-today", setup[:live] ? "live" : "unit")
    if _should_skip
...
def today_direct_setup(mockres)
