...
class TodayEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BUDDHA_QUOTES_TEST_TODAY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def today_basic_setup(extra)
  ...
  entid_env_raw = ENV["BUDDHA_QUOTES_TEST_TODAY_ENTID"]
  ...
  if env["BUDDHA_QUOTES_TEST_LIVE"] == "TRUE"
    ...
    client = BuddhaQuotesSDK.new(Helpers.to_map(merged_opts))
  end
