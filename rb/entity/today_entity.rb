...
class TodayEntity
  ...
  def make
    opts = @_entopts.dup
    TodayEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BuddhaQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = BuddhaQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Today, Hash] the loaded Today; raises BuddhaQuotesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BuddhaQuotesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
