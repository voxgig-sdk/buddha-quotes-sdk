...
type TodayEntity struct {
	name    string
	client  *core.BuddhaQuotesSDK
...
func NewTodayEntity(client *core.BuddhaQuotesSDK, entopts map[string]any) *TodayEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *TodayEntity) GetName() string { return e.name }
...
func (e *TodayEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *TodayEntity) Deleted() bool {
	return e.deleted
}


func (e *TodayEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewTodayEntity(e.client, opts)
}

func (e *TodayEntity) Data(args ...any) any {
...
func (e *TodayEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Today; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *TodayEntity) DataTyped(data ...Today) Today {
	if len(data) > 0 {
		return typedFrom[Today](e.Data(asMap(data[0])))
	}
	return typedFrom[Today](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Today (all fields
// optional at the wire level).
func (e *TodayEntity) MatchTyped(match ...Today) Today {
	if len(match) > 0 {
		return typedFrom[Today](e.Match(asMap(match[0])))
	}
	return typedFrom[Today](e.Match())
}
...
func (e *TodayEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *TodayEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// TodayLoadMatch and returns an Today. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *TodayEntity) LoadTyped(reqmatch TodayLoadMatch, ctrl map[string]any) (Today, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return Today{}, err
	}
	return typedFrom[Today](res), nil
}



func (e *TodayEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *TodayEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *TodayEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *TodayEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *TodayEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
