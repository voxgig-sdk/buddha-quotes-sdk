...
// BuddhaQuotes SDK retry feature
...
class BuddhaQuotesRetryFeature extends BuddhaQuotesBaseFeature
{
    ...
    public function init(BuddhaQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BuddhaQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BuddhaQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BuddhaQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
