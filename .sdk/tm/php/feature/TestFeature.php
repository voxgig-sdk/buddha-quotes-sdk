...
// BuddhaQuotes SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class BuddhaQuotesTestFeature extends BuddhaQuotesBaseFeature
{
    ...
    public function init(BuddhaQuotesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (BuddhaQuotesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = BuddhaQuotesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (BuddhaQuotesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(BuddhaQuotesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = BuddhaQuotesParam::call($ctx, $k);
