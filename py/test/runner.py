...
class BuddhaQuotesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BuddhaQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BuddhaQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BuddhaQuotesTestRunner.getenv("BUDDHA_QUOTES_TEST_LIVE")
        override = BuddhaQuotesTestRunner.getenv("BUDDHA_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BuddhaQuotesTestRunner.getenv(key)
        ...
        explain = BuddhaQuotesTestRunner.getenv("BUDDHA_QUOTES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BUDDHA_QUOTES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BuddhaQuotesTestRunner._test_control is not None:
            return BuddhaQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BuddhaQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BuddhaQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BuddhaQuotesTestRunner.load_test_control()
...
def load_env_local():
    BuddhaQuotesTestRunner.load_env_local()


def env_override(m):
    return BuddhaQuotesTestRunner.env_override(m)


def entity_data(v):
    return BuddhaQuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BuddhaQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BuddhaQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BuddhaQuotesTestRunner.load_test_control()


def live_delay_ms():
    return BuddhaQuotesTestRunner.live_delay_ms()
