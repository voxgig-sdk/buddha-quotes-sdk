...
from buddhaquotes_sdk.buddhaquotes_types import (
    Today,
    TodayLoadMatch,
)
...
class TodayEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TodayEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Today:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Today().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
