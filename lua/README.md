...
The Lua SDK for the BuddhaQuotes API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Random(nil):load({ id = "example_id" }, nil)
...
local result, err = client:BuddhaQuotes(nil):load(
  { id = "test01" }, nil
)
