...
local TodayEntity = {}
TodayEntity.__index = TodayEntity


function TodayEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TodayEntity)
  self._name = "today"
  self._client = client
...
function TodayEntity:get_name()
  return self._name
end


function TodayEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TodayEntity.new(self._client, opts)
end
...
function TodayEntity:mark_deleted()
  self._deleted = true
end


function TodayEntity:deleted()
  return true == self._deleted
end


function TodayEntity:data_set(args)
  if args ~= nil then
...
function TodayEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TodayEntity:match_set(args)
  if args ~= nil then
...
function TodayEntity:match_get()
...
function TodayEntity:stream(action, args, callopts)
...
---@param reqmatch TodayLoadMatch
---@param ctrl? table
---@return Today
---@return string? err
function TodayEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function TodayEntity:_run_op(ctx, post_done)
  local utility = self._utility
