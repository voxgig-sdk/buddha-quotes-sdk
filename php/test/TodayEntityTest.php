...
class TodayEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BUDDHAQUOTES_TEST_TODAY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$today_ref01_data_dt0_loaded, $err] = $today_ref01_ent->load($today_ref01_match_dt0, null);
        $this->assertNull($err);
        $this->assertNotNull($today_ref01_data_dt0_loaded);
...
function today_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BUDDHAQUOTES_TEST_TODAY_ENTID");
    ...
    if ($env["BUDDHAQUOTES_TEST_LIVE"] === "TRUE") {
        ...
        $client = new BuddhaQuotesSDK(Helpers::to_map($merged_opts));
    }
