...
class FtCtrl extends BuddhaQuotesControl
{
...
class FtClient
{
    ...
    private BuddhaQuotesUtility $utility;

    public function __construct(BuddhaQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): BuddhaQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BuddhaQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BuddhaQuotesUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BuddhaQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BuddhaQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(BuddhaQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BuddhaQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BuddhaQuotesResponse($fetched);
        }
    ...
    private function fail(BuddhaQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BuddhaQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
