...
class BuddhaQuotesSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = BuddhaQuotesHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = BuddhaQuotesHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ($utility->feature_add)($this->_rootctx, BuddhaQuotesFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return BuddhaQuotesUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = BuddhaQuotesHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = BuddhaQuotesHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = BuddhaQuotesHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new BuddhaQuotesError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = BuddhaQuotesHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new BuddhaQuotesError("graphql_error",
                "BuddhaQuotesSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Today($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_today === null) {
                $this->_today = new TodayEntity($this, null);
            }
            return $this->_today;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new BuddhaQuotesSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
