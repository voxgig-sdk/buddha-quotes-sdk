...
//     (\`new BuddhaQuotesSDK(...)\` / \`BuddhaQuotesSDK.test(...)\` become
//     \`BuddhaQuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'BuddhaQuotesSDK'
...
const TEST_SEED = {"entity":{"random":{"test01":{"id":"test01"}},"today":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new BuddhaQuotesSDK(...)\` and any
// \`BuddhaQuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BuddhaQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BuddhaQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
