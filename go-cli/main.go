// Command BuddhaQuotes-cli is a CLI + REPL for the BuddhaQuotes SDK,
...
const prompt = "buddha-quotes"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "random today"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BUDDHA_QUOTES_APIKEY (api key), BUDDHA_QUOTES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BUDDHA_QUOTES_APIKEY carries the API key and
	// BUDDHA_QUOTES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BUDDHA_QUOTES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BUDDHA_QUOTES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBuddhaQuotesSDK(opts)
