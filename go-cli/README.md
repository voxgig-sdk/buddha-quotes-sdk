...
boru-driven command-line client **and** interactive REPL for the BuddhaQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/buddha-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./buddha-quotes-cli --help

# 3. Provide credentials once, via the environment
export BUDDHA_QUOTES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./buddha-quotes-cli load 1 random            # {id:1} shorthand
./buddha-quotes-cli load '{id:1}' random       # explicit match map

# 5. Override the API base URL for a single call
BUDDHA_QUOTES_BASE=https://api.example.com ./buddha-quotes-cli load 1 random

# 6. No arguments -> interactive REPL
./buddha-quotes-cli
buddha-quotes> load 1 random
buddha-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/buddha-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/buddha-quotes-cli load 1 random
   ```
...
```sh
./buddha-quotes-cli load 1 random          # scalar shorthand for {id:1}
./buddha-quotes-cli load '{id:1}' random     # explicit match map
```
...
./buddha-quotes-cli load 1 random
```
...
Run with no arguments to open a REPL (prompt `buddha-quotes>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `random`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/buddha-quotes-cli`. |
...
(`./buddha-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
