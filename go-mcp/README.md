...
[MCP](https://modelcontextprotocol.io) server exposing the BuddhaQuotes SDK as
two agent tools — `buddha-quotes_list` and `buddha-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/buddha-quotes-mcp)
make build

# 2. Provide credentials via the environment
export BUDDHA_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user buddha-quotes \
  -- /absolute/path/to/buddha-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./buddha-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// buddha-quotes_list: first page of records
{ "entity": "random" }
{ "entity": "random", "query": { } }

// buddha-quotes_load: one record by id
{ "entity": "random", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/buddha-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user buddha-quotes \
     -- "$PWD"/dist/*/buddha-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `buddha-quotes_list` and `buddha-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list random using buddha-quotes"*
   and it calls `buddha-quotes_list` with `{"entity":"random"}`.
...
```sh
export BUDDHA_QUOTES_APIKEY=sk_live_xxx            # API key
export BUDDHA_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `buddha-quotes_list` tool
...
### Call the `buddha-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `buddha-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `buddha-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BUDDHA_QUOTES_APIKEY` | API key sent with every request. |
| `BUDDHA_QUOTES_BASE` | Optional override of the API base URL. |
...
```sh
./buddha-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"buddha-quotes_load","arguments":{"entity":"random","query":{"id":1}}}}'
```
